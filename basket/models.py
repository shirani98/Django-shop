...
class Basket(models.Model):
    ...
    def add_user (cls,self):
        if self.request.user.is_authenticated: 
                basket = get_object_or_404(cls, id = self.request.COOKIES.get('basket_id'))
    ...
    def submit(cls,self,response,quantity_form):
        ...
        BasketInline.add(self.request.POST.get('product_id'),basket.id,quantity_form)
        cooki = response.set_cookie('basket_id', basket.id) 
        return cooki
    

    
class BasketInline(models.Model):
    basket = models.ForeignKey(Basket, on_delete=models.CASCADE, related_name='basket_inline')
    ...
    def add(cls,product_id, basket_id , quantity):
        ...
        if basket_in.exists():
            ...
            cls.objects.create(basket =basket,product =product,quantity = quantity )
